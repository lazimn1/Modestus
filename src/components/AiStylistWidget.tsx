...
const EDGE_MARGIN = 12; // how close to screen edge when snapped
const DEFAULT_Y_RATIO = 0.75; // default vertical position (75% down the screen)
...
export default function AiStylistWidget() {
  ...
  // Position: { x, y } = actual pixel position of button top-left corner
  const [btnPos, setBtnPos] = useState<{ x: number; y: number } | null>(null);
  const [isSnapping, setIsSnapping] = useState(false); // triggers CSS transition for snap

  const isDragging = useRef(false);
  const hasDragged = useRef(false);
  const dragOffset = useRef({ x: 0, y: 0 }); // cursor offset within the button

  // Initialize position on mount (after we know window size)
  useEffect(() => {
    const saved = sessionStorage.getItem("mchat_btn_pos");
    if (saved) {
      try {
        const p = JSON.parse(saved);
        // Clamp to current window in case it was saved on a different size
        const clampedX = p.x < window.innerWidth / 2
          ? EDGE_MARGIN
          : window.innerWidth - BTN_SIZE - EDGE_MARGIN;
        setBtnPos({ x: clampedX, y: Math.max(EDGE_MARGIN, Math.min(p.y, window.innerHeight - BTN_SIZE - EDGE_MARGIN)) });
        return;
      } catch {}
    }
    setBtnPos({
  ...
  useEffect(() => {
    if (btnPos) sessionStorage.setItem("mchat_btn_pos", JSON.stringify(btnPos));
  }, [btnPos]);
  ...
  useEffect(() => {
    const onResize = () => {
      setBtnPos(prev => {
        if (!prev) return prev;
        const clampedX = prev.x < window.innerWidth / 2
          ? EDGE_MARGIN
          : window.innerWidth - BTN_SIZE - EDGE_MARGIN;
        return { x: clampedX, y: Math.max(EDGE_MARGIN, Math.min(prev.y, window.innerHeight - BTN_SIZE - EDGE_MARGIN)) };
      });
    };
  ...
  const snapToEdge = useCallback((currentX: number, currentY: number) => {
    const snappedX = currentX + BTN_SIZE / 2 < window.innerWidth / 2
      ? EDGE_MARGIN
      : window.innerWidth - BTN_SIZE - EDGE_MARGIN;
    const clampedY = Math.max(EDGE_MARGIN, Math.min(currentY, window.innerHeight - BTN_SIZE - EDGE_MARGIN));
    setIsSnapping(true);
    setBtnPos({ x: snappedX, y: clampedY });
  ...
  const onPointerMove = useCallback((e: React.PointerEvent) => {
    ...
    if (Math.abs(newX - (btnPos?.x ?? 0)) > 4 || Math.abs(newY - (btnPos?.y ?? 0)) > 4) {
      hasDragged.current = true;
    }
    setBtnPos({
      x: Math.max(0, Math.min(newX, window.innerWidth - BTN_SIZE)),
      y: Math.max(0, Math.min(newY, window.innerHeight - BTN_SIZE)),
    });
  }, [btnPos]);
  ...
  const onPointerUp = useCallback((e: React.PointerEvent) => {
    ...
    if (hasDragged.current && btnPos) {
      snapToEdge(btnPos.x, btnPos.y);
    }
  }, [btnPos, snapToEdge]);
  ...
  // Determine which side the button is on (for chat panel positioning)
  const isOnLeftSide = btnPos ? btnPos.x < window.innerWidth / 2 : false;
  ...
  return (
    <>
      ...
      {/* Desktop: iPhone-style floating button */}
      {btnPos && (
        <div className="hidden sm:block" style={{ position: "fixed", zIndex: 50, top: 0, left: 0, width: "100vw", height: "100vh", pointerEvents: "none" }}>
          ...
          <div
            style={{
              ...
              width: BTN_SIZE,
              height: BTN_SIZE,
              transition: isSnapping ? "left 0.35s cubic-bezier(0.34, 1.56, 0.64, 1), top 0.35s cubic-bezier(0.34, 1.56, 0.64, 1)" : "none",
              pointerEvents: "auto",
            }}
          >
            {!isOpen && (
              <button
                ...
                className="relative flex items-center justify-center w-full h-full bg-gradient-to-br from-indigo-600 via-purple-600 to-indigo-700 text-white rounded-full shadow-2xl shadow-indigo-500/40 border border-white/20 touch-none select-none"
                style={{ cursor: isDragging.current ? "grabbing" : "grab" }}
                aria-label="Open M Chat"
              >
                {/* Semi-transparent edge hint when snapped */}
                <div className="absolute inset-0 rounded-full bg-white/5 opacity-0 hover:opacity-100 transition-opacity duration-200" />
                <div className="absolute top-1 right-1 w-2.5 h-2.5 bg-green-400 rounded-full animate-pulse border-2 border-[#0a0a0a]" />
                <Sparkles className="w-6 h-6 text-indigo-200 animate-spin" style={{ animationDuration: "8s" }} />
              </button>
            )}

            {/* Open button (smaller, just an icon) */}
            {isOpen && (
              <button
                onPointerDown={onPointerDown}
                onPointerMove={onPointerMove}
                onPointerUp={onPointerUp}
                onClick={() => { if (!hasDragged.current) setIsOpen(false); }}
                className="relative flex items-center justify-center w-full h-full bg-gradient-to-br from-indigo-600 via-purple-600 to-indigo-700 text-white rounded-full shadow-2xl shadow-indigo-500/40 border border-white/20 touch-none select-none"
                style={{ cursor: isDragging.current ? "grabbing" : "grab" }}
                aria-label="Close M Chat"
              >
                <div className="absolute top-1 right-1 w-2.5 h-2.5 bg-green-400 rounded-full animate-pulse border-2 border-[#0a0a0a]" />
          ...
          {/* Chat panel — positioned relative to button, on the correct side */}
          {isOpen && btnPos && (
            <div
              style={{
                position: "absolute",
                // Place panel to the left or right of the button based on which side it's on
                left: isOnLeftSide
                  ? btnPos.x + BTN_SIZE + 12
                  : Math.max(8, btnPos.x - 420 - 12),
                // Align vertically with button, but keep within viewport
                top: Math.max(8, Math.min(btnPos.y, window.innerHeight - 620 - 8)),
                width: 420,
                pointerEvents: "auto",
              }}
              ...
              style={{ maxHeight: "min(620px, calc(100vh - 16px))" }}
            >
              {/* Header */}
              ...
              {/* Messages */}
              <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-white/10" style={{ maxHeight: 380 }}>
                {messages.map((msg) => (
